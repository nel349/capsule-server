...
export class ContentValidator {
  ...
  static isValidIPFSCID(cid: string): boolean {
    ...
    return /^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]+$/.test(cid.slice(2));
  }
  ...
  static validateCapsuleContent(
    ...
    if (contentStorage.type === 'Text' && encryptedContent.length > 280) {
      return { valid: false, error: 'Text content too long for on-chain storage' };
    }
...
export interface ContentStorage {
  type: 'Text' | 'Document' | 'SocialArchive' | 'MediaBundle' | 'ExternalWithBackup';
  cid?: string;
  originalUrl?: string;
  archivedCid?: string;
  platform?: string;
