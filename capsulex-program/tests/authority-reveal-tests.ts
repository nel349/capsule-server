...
function getDefaultAccounts({ provider, capsulePda, nftMintPda, vaultPda }: any) {
  return {
...
describe("Authority Reveal Tests", () => {
  ...
  before(async () => {
    ...
    console.log(`Note: Using provider wallet as app authority for testing purposes`);
  });
  ...
  it("🏗️ Initialize Program (if not already done)", async () => {
    try {
      ...
      console.log("Program already initialized (or other error):", error.message);
    }
  });
  ...
  it("✅ Creator can reveal their own capsule (existing functionality)", async () => {
    ...
    const capsulePda = getCapsulePda(regularUser.publicKey, revealDate, program.programId);
    ...
    await program.methods.revealCapsule(revealDate)
      .accounts({
    ...
    expect(capsule.creator.toBase58()).to.equal(regularUser.publicKey.toBase58());
  ...
  it("❌ Cannot reveal capsule before reveal date (even as app authority)", async () => {
    ...
    try {
      ...
      console.log("✅ Correctly rejected early reveal attempt by app authority");
    }
  ...
  it("🔍 Verify app authority setup for testing", async () => {
    ...
    const productionAppAuthority = "FnnLFxD5jZh9dhMbPBYvon3nBWm2gmJDaQnXJhYD2G12";
