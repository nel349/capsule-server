...
describe("CapsuleX Game Instructions", () => {
  ...
  it("Game: Initialize game with correct parameters", async () => {
    ...
    expect(game.creator.toBase58()).to.equal(
      capsuleCreator.publicKey.toBase58()
    );
  ...
  it("Game: Submit guesses with anonymity options", async () => {
    ...
    await program.methods
      .submitGuess("public guess", false)
      .accounts({
    ...
    await program.methods
      .submitGuess("anonymous guess", true)
      .accounts({
        guesser: gamePlayer.publicKey,
        game: gamePda,
        guess: paidPda,
  ...
  it("Game: Multiple users submit guesses", async () => {
    ...
    const [guess1Pda] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("guess"),
        gamePda.toBuffer(),
        gamePlayer1.publicKey.toBuffer(),
    ...
    const [guess2Pda] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("guess"),
        gamePda.toBuffer(),
        gamePlayer2.publicKey.toBuffer(),
    ...
    expect(guess1.guesser.toBase58()).to.equal(
      gamePlayer1.publicKey.toBase58()
    );
    ...
    expect(guess2.guesser.toBase58()).to.equal(
      gamePlayer2.publicKey.toBase58()
    );
  ...
  it("Game: Verify guess after capsule reveal (delayed verification)", async () => {
    ...
    expect(finalGame.winner.toBase58()).to.equal(
      gamePlayer.publicKey.toBase58()
    );
  ...
  it("Game: Distribute rewards after winner found", async () => {
    ...
    const [guessPda] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("guess"),
        gamePda.toBuffer(),
        gamePlayerWinner.publicKey.toBuffer(),
        Buffer.from([0, 0, 0, 0]),
      ],
      program.programId
    );
    ...
    await program.methods
      .initializeLeaderboard(gamePlayerWinner.publicKey)
      .accounts({
    ...
    try {
      await program.methods
        .initializeLeaderboard(capsuleCreator.publicKey)
        .accounts({
    ...
    const leaderboard = await program.account.leaderboardEntry.fetch(
      leaderboardPda
    );
      ...
      for (let i = 0; i < 2; i++) {
        const [guessPda] = PublicKey.findProgramAddressSync(
          [
            Buffer.from("guess"),
            gamePda.toBuffer(),
            capsuleCreator.publicKey.toBuffer(),
            Buffer.from([i, 0, 0, 0]),
          ],
          program.programId
        );
