...
async function initializeProgram() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   Total Fees Collected: ${existingVault.totalFeesCollected.toNumber()}`
      );
      return;
    } catch (fetchError) {
      // Vault doesn't exist, proceed with initialization
      console.log(
        "🔧 Program not initialized yet, proceeding with initialization..."
      );
    }
    ...
    console.log(
      `   Total Fees Collected: ${vault.totalFeesCollected.toNumber()}`
    );
    console.log(`   Vault PDA: ${vaultPda.toBase58()}`);
  } catch (error) {
